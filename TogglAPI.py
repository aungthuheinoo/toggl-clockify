...
class TogglAPI:
    ...
    def getWorkspaceTags(self, workspaceName):
        if self._syncTags == True:
            ...
            url = "https://www.toggl.com/api/v8/workspaces/%d/tags"%wsId
            # New added Line for variable declare
    ...
    def getWorkspaceGroups(self, workspaceName):
        if self._syncGroups == True:
            self.groups = []
            wsId = self.getWorkspaceID(workspaceName)
            url = "https://www.toggl.com/api/v8/workspaces/%d/groups"%wsId
            req = self._request(url)
            if req.ok:
    ...
    def getWorkspaceUsers(self, workspaceName):
        if self._syncUsers == True:
            wsId = self.getWorkspaceID(workspaceName)
            url = "https://www.toggl.com/api/v8/workspaces/%d/users"%wsId
            req = self._request(url)
            if req.ok:
                self.users = req.json()
            else:
    ...
    def getWorkspaceClients(self, workspaceName):
        if self._syncClients == True:
            wsId = self.getWorkspaceID(workspaceName)
            url = "https://www.toggl.com/api/v8/workspaces/%d/clients"%wsId
            req = self._request(url)
    ...
    def getWorkspaceProjects(self, workspaceName):
        if self._syncProjects == True:
            wsId = self.getWorkspaceID(workspaceName)
            url = "https://www.toggl.com/api/v8/workspaces/%d/projects"%wsId
            params = {"active": "both"}
    ...
    def getWorkspaceTasks(self, workspaceName):
        if self._syncTasks == True:
            wsId = self.getWorkspaceID(workspaceName)
            url = "https://www.toggl.com/api/v8/workspaces/%d/tasks"%wsId
            req = self._request(url)
            self.tasks = req.json()
